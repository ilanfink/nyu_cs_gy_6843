...
def receiveOnePing(mySocket, ID, timeout, destAddr):
    ...
    while 1:
        ...
        if packetID == ID:
            ...
            time_sent = struct.unpack("d", recPacket[28:28 + size])[0]
            time_difference = timeReceived - time_sent
            print(f"Reply from {addr[0]}: bytes={size} time={time_difference:7f}ms TTL={timeLeft}")
